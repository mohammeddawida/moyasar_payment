...
class MoyasarPayment {
  ...
  Future _pay(
    ...
    var headers = <String, String>{'Content-type': 'application/json', 'Accept': 'application/json', 'Authorization': 'Basic ' + base64Encode(utf8.encode(publishableKey + ':'))};
  ...
  Future applePay({
    ...
    var flutterPay = FlutterPay();
    flutterPay.setEnvironment(environment: PaymentEnvironment.Production);
    var token = await flutterPay.requestPayment(
                  appleParameters: AppleParameters(merchantIdentifier:applepayMerchantId),
                  currencyCode: currencyCode.toUpperCase(),
                  countryCode: countryCode.toUpperCase(),
                  paymentItems: paymentItemsList,
                );
    var source = {'type': 'applepay', 'token': token};      
    return _pay(description: description, amount: amount, publishableKey: publishableKey, source: source);
  }
  /// # Credit Card
  ...
  Future creditCard(
    ...
    var source = {'type': 'creditcard', 'name': cardHolderName, 'number': int.parse(cardNumber), 'cvc': int.parse(cvv), 'month': expiryManth, 'year': expiryYear};
    
    return _pay(description: description, amount: amount, publishableKey: publishableKey,source: source,callbackUrl:callbackUrl);
  }
  /// # STC Pay
  ...
  Future stcPayOtp(
    ...
    var data = await http.get(Uri.parse(transactionUrl + '&otp_value=' + otpValue));
    return data;
  }
